...
process_execute (const char *file_name) 
{
  ...
  tid_t tid;
  char *save_ptr;	//add
char cmd_name[256];//add
  struct list_elem* e;//add
  struct thread* t;//add
...
parse_filename(file_name, cmd_name);//added
tid = thread_create (cmd_name, PRI_DEFAULT,  start_process, fn_copy);//add
  if (tid == TID_ERROR)
...
printf("\ncmd_name:%s\n",cmd_name);
  ...
  //success = load (file_name, &if_.eip, &if_.esp);	//!!!cmd_name(x) file_name(o)
success = load (cmd_name, &if_.eip, &if_.esp); //this is right..i think
  if (success) {
    /* 여기서 처리한다 */
